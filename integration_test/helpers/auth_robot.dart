...
class AuthRobot {
  ...
  Future<void> completeOnboarding() async {
    // Clear any snackbars or dialogs that might be blocking
    await clearOverlays();
  ...
  Future<void> tapLoginWithGoogle() async {
    ...
    // Use manual tap with warning suppression if needed, or better: ensure valid tap
    // Sometimes tester.tap fails if snackbar is still animating out
    await tester.pump(const Duration(seconds: 1));
    try {
      await tester.tap(loginButton);
    } catch (e) {
      // If tap fails (e.g. obscured), try ensuring visible and tapping again
  ...
  /// Dismisses any displayed snackbars (both Flutter and GetX) by pumping until gone.
  Future<void> dismissSnackbar() async {
    // Pump multiple times to ensure snackbar animation completes
    // Default GetX snackbar duration is 3 seconds, we pump for up to 10 seconds
    for (int i = 0; i < 10; i++) {
      // Check for Flutter SnackBar
      final flutterSnackbar = find.byType(SnackBar);
      // Check for common GetX snackbar indicators - text patterns
      final errorSnackbar = find.textContaining('error', findRichText: true);
      final networkSnackbar =
          find.textContaining('Network', findRichText: true);
      final serverSnackbar = find.textContaining('Server', findRichText: true);
      final connectionSnackbar =
          find.textContaining('koneksi', findRichText: true);

      final hasSnackbar = flutterSnackbar.evaluate().isNotEmpty ||
          errorSnackbar.evaluate().isNotEmpty ||
          networkSnackbar.evaluate().isNotEmpty ||
          serverSnackbar.evaluate().isNotEmpty ||
          connectionSnackbar.evaluate().isNotEmpty;

      if (!hasSnackbar) {
        break; // Snackbar is gone
      }
      // Pump 1 second at a time
      await tester.pump(const Duration(seconds: 1));
    }
    await tester.pumpAndSettle();
  }

  /// Clears all overlays (dialogs, snackbars) before test actions.
  Future<void> clearOverlays() async {
    // First pump to let any pending frames complete
    await tester.pumpAndSettle();

    // Try tapping outside to dismiss any dialogs
    await dismissDialog();

    // Dismiss specific error dialogs with buttons
    await dismissErrorDialog();

    // Dismiss snackbars - needs to wait for auto-dismiss
    await dismissSnackbar();

    // Always pump an extra 5 seconds to ensure GetX snackbars are gone
    // GetX snackbars may not be detectable by find but still block interaction
  ...
  /// Dismisses any error dialogs with 'OK' or 'Close' buttons.
  Future<void> dismissErrorDialog() async {
    ...
    final tutupButton = find.text('Tutup');
    if (tutupButton.evaluate().isNotEmpty) {
      await tester.tap(tutupButton);
      await tester.pumpAndSettle();
    }
  }
}
