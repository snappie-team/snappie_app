...
class MockArticlesRepository extends GetxService implements ArticlesRepository {
  @override
...
class MockPlaceRepository extends GetxService implements PlaceRepository {
  final List<PlaceModel> _dummyPlaces = [
    PlaceModel()
      ..id = 1
      ..name = 'Warung Nasi Goreng Gila'
      ..avgRating = 4.5
      ..imageUrls = [PlaceImage()..url = 'https://example.com/nasigoreng.jpg']
      ..description = 'Nasi goreng super pedas dan enak.'
      ..foodType = ['Menu Komposit', 'Makanan Tradisional']
      ..placeValue = ['Murah', 'Enak'],
    PlaceModel()
      ..id = 2
      ..name = 'Kafe Kenangan'
      ..avgRating = 4.8
      ..imageUrls = [PlaceImage()..url = 'https://example.com/kopi.jpg']
      ..description = 'Tempat nongkrong asik dengan kopi pilihan.'
      ..foodType = ['Minuman dan Tambahan']
      ..placeValue = ['Nyaman', 'Instagrammable'],
    PlaceModel()
      ..id = 3
      ..name = 'Restoran Mewah'
      ..avgRating = 4.9
      ..imageUrls = [PlaceImage()..url = 'https://example.com/steak.jpg']
      ..description = 'Pengalaman makan malam yang tak terlupakan.'
      ..foodType = ['Makanan Cepat Saji', 'Gaya Tionghoa']
      ..placeValue = ['Mewah', 'Romantis'],
  ];
  ...
  Future<List<PlaceModel>> getPlaces({
    ...
    var result = _dummyPlaces;

    // Filter by search query (name or foodType)
    if (search != null && search.isNotEmpty) {
      final query = search.toLowerCase();
      result = result.where((place) {
        final nameMatch = place.name?.toLowerCase().contains(query) ?? false;
        final typeMatch =
            place.foodType?.any((t) => t.toLowerCase().contains(query)) ??
                false;
    ...
    if (foodTypes != null && foodTypes.isNotEmpty) {
      result = result.where((place) {
        return place.foodType?.any((t) => foodTypes.contains(t)) ?? false;
      }).toList();
    }

    if (placeValues != null && placeValues.isNotEmpty) {
      result = result.where((place) {
        return place.placeValue?.any((v) => placeValues.contains(v)) ?? false;
      }).toList();
    }

    if (minRating != null) {
      result =
          result.where((place) => (place.avgRating ?? 0) >= minRating).toList();
    }
  ...
  Future<PlaceModel> getPlaceById(int id) async {
    ...
    return _dummyPlaces.firstWhere(
      (place) => place.id == id,
      orElse: () => PlaceModel()
        ..id = id
        ..name = 'Unknown Place',
